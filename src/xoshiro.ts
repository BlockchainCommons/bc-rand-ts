...
export class Xoshiro256StarStar {
  ...
  fillBytes(dest: Uint8Array): void {
    let i = 0;
    for (; i + 8 <= dest.length; i += 8) {
      this.step();
      dest[i] = this.outLo & 0xff;
      dest[i + 1] = (this.outLo >>> 8) & 0xff;
      dest[i + 2] = (this.outLo >>> 16) & 0xff;
      dest[i + 3] = this.outLo >>> 24;
      dest[i + 4] = this.outHi & 0xff;
      dest[i + 5] = (this.outHi >>> 8) & 0xff;
      dest[i + 6] = (this.outHi >>> 16) & 0xff;
      dest[i + 7] = this.outHi >>> 24;
    }
    if (i < dest.length) {
      const v = this.nextU64();
      for (let j = 0; i + j < dest.length; j++) dest[i + j] = Number((v >> BigInt(j * 8)) & 0xffn);
    }
  }
}
