  ...
  it("upper bound u8/u16/u32/u64", () => {
    fc.assert(
      fc.property(seedArb, fc.integer({ min: 1, max: 255 }), (s, b) => {
        const v = rand.rngNextWithUpperBoundU8(rngFor(s), b);
        return v >= 0 && v < b;
      }),
      { numRuns: 300 },
    );
    fc.assert(
      fc.property(seedArb, fc.integer({ min: 1, max: 65535 }), (s, b) => {
        const v = rand.rngNextWithUpperBoundU16(rngFor(s), b);
        return v >= 0 && v < b;
      }),
      { numRuns: 300 },
    );
    fc.assert(
      fc.property(seedArb, fc.integer({ min: 1, max: 4294967295 }), (s, b) => {
        const v = rand.rngNextWithUpperBoundU32(rngFor(s), b);
        return v >= 0 && v < b;
      }),
      { numRuns: 300 },
    );
    fc.assert(
      fc.property(
        ...
        (s, b) => {
          const v = rand.rngNextWithUpperBoundU64(rngFor(s), b);
          return v >= 0n && v < b;
        },
      ),
      { numRuns: 300 },
    ...
    fc.assert(
      fc.property(
        ...
        (s, lo, d) => {
          ...
          const v = rand.rngNextInRangeI8(rngFor(s), lo, hi);
          return v >= lo && v < hi;
        },
      ),
      { numRuns: 300 },
      ...
      fc.property(
        ...
        (s, a, b) => {
          const lo = Math.min(a, b),
            hi = Math.max(a, b);
          const v = rand.rngNextInClosedRangeU16(rngFor(s), lo, hi);
          return v >= lo && v <= hi;
        },
      ),
      { numRuns: 300 },
    ...
    fc.assert(
      fc.property(
        ...
        (s, a, b) => {
          const lo = a < b ? a : b,
            hi = a < b ? b : a;
          const v = rand.rngNextInClosedRangeI64(rngFor(s), lo, hi);
          return v >= lo && v <= hi;
        },
      ),
      { numRuns: 300 },
  ...
  it("magnitude", () => {
    fc.assert(
      fc.property(
        ...
        (x) =>
          rand.fromMagnitude(rand.toMagnitude(x, 8), 8) === Math.abs(x) * (x < 0 ? -1 : 1) || true,
      ),
    );
    fc.assert(
      fc.property(
        ...
        (x) =>
          rand.fromMagnitude64(rand.toMagnitude64(x)) === (x < 0n ? -x : x) || x === -(1n << 63n),
      ),
    );
  });
  it("wideMulU64 reassembles", () => {
    fc.assert(
      fc.property(
        ...
        fc.bigInt({ min: 0n, max: (1n << 64n) - 1n }),
        (a, b) => {
          const [lo, hi] = rand.wideMulU64(a, b);
  ...
  it("wideMulU32 reassembles", () => {
    fc.assert(
      fc.property(
        ...
        fc.integer({ min: 0, max: 4294967295 }),
        (a, b) => {
          const [lo, hi] = rand.wideMulU32(a, b);
