...
describe("samplers stay in bounds", () => {
  ...
  // Tombstone T1 landed: i64 ranges with a negative start are in range.
  it("i64 ranges with a negative start (T1 fixed)", () => {
  ...
  it("magnitude", () => {
    fc.assert(
      fc.property(
        ...
        (x) =>
          samplers.fromMagnitude(samplers.toMagnitude(x, 8), 8) ===
    ...
    fc.assert(
      fc.property(
        ...
        (x) =>
          samplers.fromMagnitude64(samplers.toMagnitude64(x)) === (x < 0n ? -x : x) ||
          x === -(1n << 63n),
      ),
    );
  });
  it("wideMulU64 reassembles", () => {
    fc.assert(
      fc.property(
        ...
        fc.bigInt({ min: 0n, max: (1n << 64n) - 1n }),
        (a, b) => {
          const [lo, hi] = samplers.wideMulU64(a, b);
  ...
  it("wideMulU32 reassembles", () => {
    fc.assert(
      fc.property(
        ...
        fc.integer({ min: 0, max: 4294967295 }),
        (a, b) => {
          const [lo, hi] = samplers.wideMulU32(a, b);
